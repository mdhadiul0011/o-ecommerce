  ...
  return (
    <div className='mb-7'>
...

      {
        ...
        <div>
          {data.map((item, i)=>(
              <SubDropDown key={i} catagoryDrop={item.subCatagory ? true : false} title={item.name} color=''>
                {item.subCatagory && 
                  item.subCatagory.map((items, i)=>(
                    <p key={i} className='font-primaryFont font-normal text-base text-gray cursor-pointer mt-5 border-b border-solid border-gray pl-3 pb-4'>{items.name}</p>
                  ))
                }
            </SubDropDown>    
          ))}
